...
#
#
# SDR DVB-S2 Transmitter
#
# script file to generate a transport stream with the internal camera at a given symbol rate :
# 125, 250, 500, 1000 or 2000 at a fix FEC = 9/10
# send the SR and FEC parameters to the gnuradio DVB-S Transmitter (dvbs2_tx_var.py)
# 
# requirements :
#
...
ID_TS="1000"
ID_SERVICE="1"
SERVICE_PROVIDER="GNURadio-DVB-S2"
SERVICE_NAME="HB9DUG"
...
echo "----------------------------------------------------------------------------"
echo "DVB-S2 8PSK - Symbol rate [KS/s]: $SYMBOL_RATE  FEC: $FEC  TS rate [Mb/s]: $RATE_MUX"
echo "----------------------------------------------------------------------------"
...
./dvbs2_8psk_tx_var_v1.py -sr $SYMBOL_RATE_NUM -vr $FEC >/dev/null 2>/dev/null &
